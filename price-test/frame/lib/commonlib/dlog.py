...
class _dlog(object):
    def __init__(self, logger_name="dlog", tag_name=""):
    ...
    def init_logger(self, logpath, mode="w", force=False):
        ...
        if logdir:
            from frame.lib.commonlib.dtssystem import dtssystem     # 避免循环import，因此放在函数内import
            cmd = "mkdir -p " + logdir
            dtssystem(cmd)
...
# dlog 全局句柄，默认设置屏幕输出
dlog = LogProxy(_dlog())
dlog.init_stream_handler()


def _test():
    dlog.init_logger("./test.log")
    ...
    try:
        ...
        dlog.warning("warning %s", "日志", exc_info=True)
    dlog.diagnose("error %s日志", "格式字符串")
    dlog.critical("critical 日志")
