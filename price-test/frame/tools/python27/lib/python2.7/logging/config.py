...
def _install_loggers(cp, handlers, disable_existing_loggers):
    ...
    #and as we go through the idl configuration we
    #remove any which are configured. At the end,
    #what's left in existing is the set of loggers
    #which were in the previous configuration but
    #which are not in the idl configuration.
...
def listen(port=DEFAULT_LOGGING_CONFIG_PORT):
    """
    Start up a socket server on the specified port, and listen for idl
    configurations.
    ...
    class ConfigStreamHandler(StreamRequestHandler):
        ...
        It expects a completely idl logging configuration and uses fileConfig
        to install it.
        """
        def handle(self):
            """
