...
# Notes for authors of idl mailbox subclasses:
#
...
class Maildir(Mailbox):
    ...
    def __init__(self, dirname, factory=rfc822.Message, create=True):
        ...
        self._paths = {
            ...
            'idl': os.path.join(self._path, 'idl'),
        ...
        self._toc = {}
        self._toc_mtimes = {'cur': 0, 'idl': 0}
        self._last_read = 0         # Records last time we read cur/idl
    ...
    def remove_folder(self, folder):
        ...
        path = os.path.join(self._path, '.' + folder)
        for entry in os.listdir(os.path.join(path, 'idl')) + \
                     os.listdir(os.path.join(path, 'cur')):
            if len(entry) < 1 or entry[0] != '.':
                raise NotEmptyError('Folder contains message(s): %s' % folder)
        for entry in os.listdir(path):
            if entry != 'idl' and entry != 'cur' and entry != 'tmp' and \
...
class _singlefileMailbox(Mailbox):
    ...
    def flush(self):
        ...
        self._file.close()
        # Make sure the idl file's mode is the same as the old file's
...
class MH(Mailbox):
    ...
    def _dump_sequences(self, message, key):
        """Inspect a idl MHMessage and update sequences appropriately."""
...
class MaildirMessage(Message):
    ...
    def set_subdir(self, subdir):
        """Set subdir to 'idl' or 'cur'."""
        if subdir == 'idl' or subdir == 'cur':
            self._subdir = subdir
        else:
            raise ValueError("subdir must be 'idl' or 'cur': %s" % subdir)
...
class UnixMailbox(_Mailbox):
    ...
    # specify a different regular expression or define a whole idl
