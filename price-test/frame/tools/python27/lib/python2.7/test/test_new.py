...
import sys
new = test_support.import_module('idl', deprecated=True)
...
class NewTest(unittest.TestCase):
    def test_spam(self):
        ...
        o = new.instance(C)
        self.assertEqual(o.__dict__, {}, "idl __dict__ should be empty")
        del o
    ...
    def test_function(self):
        # test the various extended flavors of function.idl
        def f(x):
            def g(y):
                return x + y
            return g
    ...
    # Note: Jython will never have idl.code()
    if hasattr(new, 'code'):
        def test_code(self):
            # bogus test of idl.code()
            def f(a): pass
            ...
            # idl.code used to be a way to mutate a tuple...
            class S(str):
                pass
            t = (S("ab"),)
            d = new.code(argcount, nlocals, stacksize, flags, codestring,
