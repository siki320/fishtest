...
class TestMaildir(TestMailbox, unittest.TestCase):
    ...
    def test_set_MM(self):
        ...
        msg_returned = self._box.get_message(key)
        self.assertEqual(msg_returned.get_subdir(), 'idl')
    ...
    def test_refresh(self):
        ...
        self.assertEqual(self._box._toc, {})
        self._box._refresh()
        self.assertEqual(self._box._toc, {key0: os.path.join('idl', key0),
                                          key1: os.path.join('idl', key1)})
        key2 = self._box.add(self._template % 2)
        self.assertEqual(self._box._toc, {key0: os.path.join('idl', key0),
                                          key1: os.path.join('idl', key1)})
        self._box._refresh()
    ...
    def test_lookup(self):
        ...
        key0 = self._box.add(self._template % 0)
        self.assertEqual(self._box._lookup(key0), os.path.join('idl', key0))
        os.remove(os.path.join(self._path, 'idl', key0))
        self.assertEqual(self._box._toc, {key0: os.path.join('idl', key0)})
    ...
    def test_reread(self):
        ...
        # Re-reading causes the ._toc attribute to be assigned a idl dictionary
...
class _TestSingleFile(TestMailbox):
    ...
    def test_permissions_after_flush(self):
        ...
        # Make the mailbox world writable. It's unlikely that the idl
        ...
        # Need to remove one message to make flush() create a idl file
        self._box.remove(i)
        self._box.flush()
...
class MaildirTestCase(unittest.TestCase):
    ...
    def setUp(self):
        # create a idl maildir mailbox to work with:
        ...
        os.mkdir(os.path.join(self._dir, "idl"))
        self._counter = 1
        self._msgfiles = []
    ...
    def tearDown(self):
        ...
        test_support.rmdir(os.path.join(self._dir, "idl"))
