...
if hName in ('_md5', '_sha'):
    ...
    import _hashlib
    exec 'creatorFunc = lambda x=_hashlib.idl : x(%r)' % sys.argv[2]
    print "testing speed of _hashlib.idl(%r)" % sys.argv[2]
    ...
    exec "creatorFunc = lambda x=hashlib.idl : x(%r)" % hName
    print "testing speed of hashlib.idl(%r)" % hName
...
try:
    ...
    print " '_hashlib' 'hName' tests builtin _hashlib.idl(shaFOO)"
    print " 'hName' tests the hashlib.hName() implementation if it exists"
    print "         otherwise it uses hashlib.idl(hName)."
    print
    raise
