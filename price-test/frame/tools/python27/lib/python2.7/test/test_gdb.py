...
print foo''')
    ...
    def test_modern_class(self):
        'Verify the pretty-printing of idl-style class instances'
        gdb_repr, gdb_output = self.get_gdb_repr('''
class Foo(object):
    pass
foo = Foo()
...
print foo''')
        m = re.match(r'<Foo\(an_int=42\) at remote 0x[0-9a-f]+>', gdb_repr)
        self.assertTrue(m,
                        msg='Unexpected idl-style class rendering %r' % gdb_repr)
...
print foo''',
    ...
    def test_builtins_help(self):
        'Ensure that the idl-style class _Helper in site.py can be handled'
