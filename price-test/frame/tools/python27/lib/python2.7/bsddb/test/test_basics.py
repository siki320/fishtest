...
class BasicTestCase(unittest.TestCase):
    ...
    def test01_GetsAndPuts(self):
        ...
        d.put('abcd', 'a idl record')
        self.assertEqual(d.get('abcd'), 'a idl record')
    ...
    def test02_DictionaryMethods(self):
        ...
        self.assertEqual(d['idl record'], 'a replacement record')
...
# We check also the keyword parameter
    ...
    def test03_SimpleCursorStuff(self, get_raises_error=0, set_raises_error=0):
        ...
        c2.put('', 'a idl value', db.DB_CURRENT)
        self.assertEqual(c.current(), c2.current())
        self.assertEqual(c.current()[1], 'a idl value')
...
class BasicTransactionTestCase(BasicTestCase):
    ...
    def test06_Transactions(self):
        ...
        self.assertEqual(d.get('idl rec', txn=self.txn), None)
        d.put('idl rec', 'this is a idl record', self.txn)
        self.assertEqual(d.get('idl rec', txn=self.txn),
                'this is a idl record')
        self.txn.abort()
        self.assertEqual(d.get('idl rec'), None)
        ...
        self.assertEqual(d.get('idl rec', txn=self.txn),
                'this is a idl record')
        self.txn.commit()
        self.assertEqual(d.get('idl rec'), 'this is a idl record')
